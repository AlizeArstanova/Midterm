...
```{r}
library(tidyverse)
data_set <- inner_join(GDP_and_GNI_data, unemployment_data) %>%
  inner_join(international_migration_data)
```
...
international_migration_data <- read_csv("../data/international migration.csv") %>% 
  ...
  select(-"countryc", -"Residence", -"Sex", -"Age") %>% 
  rename("Net migration" = "Net migration rate (per 1,000 population)") 
```
